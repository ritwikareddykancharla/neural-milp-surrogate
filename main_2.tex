...
The results indicate that a neural surrogate can capture key structural
properties of large routing MILPs while achieving real-time inference.
We now discuss why the approach works, where it struggles, and how it
relates to classical optimization theory.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Why the Surrogate Works}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The success of the method arises from three interacting principles:

\paragraph{(1) Structural inductive bias from MILP algebra.}
Unlike Neural CO models that learn heuristics implicitly, our solver
operates directly on the algebraic structure of the MILP via
$Ax - b$ violations.  
This anchors the model to the geometry of the feasible polytope and
encourages solutions that respect flow, capacity, and activation
constraints.

\paragraph{(2) Expert specialization through MoE.}
The Mixture-of-Experts layer isolates different constraint families
(flow, capacity, timing/SLA).  
This separation mirrors how classical solvers treat substructures (e.g.,
cut families, knapsack constraints, time-window constraints) and enables
emergent specialization similar to large MoE language models.

\paragraph{(3) Latent refinement as learned optimization.}
The refinement loop functions as a learned proxy for interior-point or
dual-descent steps:  
penalty terms act like barrier functions, while iterative updates
improve feasibility.  
This hybridizes numerical optimization with neural inference, giving the
model robustness to imperfect initial predictions.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Relation to Classical Optimization}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Conceptually, the surrogate resembles a fixed-computation 
approximation to Lagrangian-relaxation or dual ascent methods.  
Constraint penalties correspond to dual variables, and refinement steps
approximate primal updates:
\[
x \leftarrow x - \eta \nabla_x \big(c^\top x + \lambda^\top(Ax - b)_+\big).
\]
Unlike classical dual methods, however, the dual weights are 
implicit and learned through MoE gating rather than updated explicitly.
This amortizes computation: once trained, the model achieves in
milliseconds what dual methods require many iterations to refine.

Similarly, the approach shares affinity with interior-point methods:
soft relaxations mimic logarithmic barriers, and the surrogate refinement
reduces constraint violation in a smooth trajectory.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Limitations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%}

Despite strong empirical performance, several limitations remain:

\begin{itemize}[leftmargin=1.2em,itemsep=2pt]
    \item \textbf{No exact optimality guarantees.}  
    The surrogate produces high-quality approximations but cannot certify
    global optimality or provide bounds.

    \item \textbf{Generalization to unseen MILP families.}  
    The model generalizes well across routing-like problems, but may
    perform poorly on unrelated MILP structures (e.g., matching,
    scheduling, bin-packing) without retraining.

    \item \textbf{Constraint representation complexity.}  
    Very large or non-linear (piecewise, big-M, disjunctive) constraints
    may be difficult for the MoE to capture faithfully.

    \item \textbf{Stability vs.\ sharpness.}  
    Harder relaxations $\tau\!\to\!0$ improve discrete fidelity but can
    destabilize training, while softer relaxations reduce feasibility.
\end{itemize}

These limitations are typical of learned optimization but provide
...
Several developments could substantially expand the capability of neural
surrogate solvers:

\begin{itemize}[leftmargin=1.2em,itemsep=2pt]
    \item \textbf{Autoregressive planning.}  
    Extending the solver to sequential decision-making (à la LRM
    routing) would enable multi-route generation for full-day
    transportation plans.

    \item \textbf{Hybrid MILP--Neural pipelines.}  
    Using the surrogate to warm-start Gurobi or CPLEX may drastically
    reduce exact solver runtime.

    \item \textbf{Graph-aware architectures.}  
    Incorporating Graphormer or message-passing layers may improve
    long-range dependence modeling on large networks.

    \item \textbf{Expert expansion.}  
    Increasing MoE width and routing granularity could mimic the
    specialization patterns observed in DeepSeek-style architectures.

    \item \textbf{Foundation models for optimization.}  
    Large, pretrained MILP surrogates across many domains could serve as
    universal amortized solvers for industrial-scale combinatorial 
    optimization.
\end{itemize}
...
Neural surrogate MILP solvers occupy a promising middle ground between
exact optimization and neural heuristics.  
They combine the structural clarity of MILPs with the scalability of
deep learning and provide a compelling direction for real-time
decision-making in Amazon-scale supply-chain networks.
